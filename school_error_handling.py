    ...
    def assign_grade(self, course, grade):
        try:
         if course in self.courses:
            ...
            raise ValueError(f"{self.name} is not enrolled in {course.name}")
        except ValueError as e:
            print(e)
        ...
        try:
         for teacher in self.teachers:
            if teacher.employee_id == employee_id:
                return teacher
         raise LookupError(f"No Teacher With ID {employee_id}")  
        except LookupError as e:
          print(e)
          return None
        ...
        try:
            for course in self.courses:
                if course.course_id==course_id:
                 return course
            raise LookupError(f"No Courses with this ID {course_id}")
        except LookupError as e:
            print(e)
            return None
...
def generate_demo_data(school):
    ...
    course1 = school.courses[1]
    student.assign_grade(course1, 90)
    course1 = school.courses[2]
    student.assign_grade(course1, 60)
