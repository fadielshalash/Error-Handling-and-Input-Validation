...
class Student(Person):
    ...
    def enroll(self, course):
        if not isinstance(course,Course):
            raise TypeError(f"{course.name} is not a Course in the schedule")
...
class Course:
    ...
    def assign_teacher(self, teacher):
        if not isinstance(teacher, Teacher):
            raise TypeError(f"The provided object {teacher} is not a valid Teacher instance.")
        self.teacher = teacher

    def add_student(self, student):
        if not isinstance(student, Student):
         raise TypeError(f"The provided object {student} is not a valid Student instance.")
    ...
    def add_schedule(self, day, time):
        ...
        if day not in valid_days:
            raise ValueError(f"In this day there is no lessons {day}")
        try:
            datetime.strptime(time,"%H:%M")
        except ValueError:
            raise ValueError(f"Invalid Time Format Please Enter the Valid Time Format {time}")
...
class School:
    ...
    def get_teacher_by_id(self, employee_id):
        if not isinstance(employee_id,str):
            return ValueError(f"Invalid Employee ID/ Teacher ID {employee_id}")
...
def generate_demo_data(school):
    ...
    # assign teachers 
    teacher1 = school.teachers[0]
    teacher1.assign_course({"name": "Chemistry", "course_id": 3})
